...
\begin{itemize}
    \item \textbf{Backend}: Go
        \begin{itemize}
            \item \texttt{crypto/rand} - generare cheie criptografic securizată (CSPRNG)
            \item API REST pentru comunicare cu frontend
        \end{itemize}
...
\subsection{Implementare OTP în Go}
...
\begin{itemize}
    \item Folosirea \texttt{crypto/rand} din Go pentru generarea cheilor (CSPRNG)
...
\begin{enumerate}
    ...
    \item Generare securizată a cheilor folosind \texttt{crypto/rand} din Go
...
\begin{enumerate}
    ...
    \item \textbf{Generare securizată de chei}: folosiți CSPRNG (\texttt{crypto/rand} din Go)
