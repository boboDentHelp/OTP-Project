...
\begin{frame}{Arhitectura Aplicației}
    \begin{center}
        \begin{tikzpicture}[
            ...
            \node[box, fill=purple!30, right of=frontend, xshift=3cm] (backend) {Backend Go\\crypto/rand};

            \draw[->, thick, <->] (frontend) -- node[above] {API REST} (backend);
        \end{tikzpicture}
    \end{center}
    ...
    \begin{itemize}
        \item \textbf{Backend Go} - server API pentru criptare
        \item \textbf{crypto/rand} - generare cheie criptografic securizată (CSPRNG)
        \item \textbf{React 19 + Tailwind} - interfață utilizator
        \item \textbf{Hooks} - useState, useCallback pentru state management
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Implementare OTP în Go}
\begin{lstlisting}[language=Go, caption=Funcții principale OTP în backend]
// generare cheie aleatorie cu crypto/rand (CSPRNG)
...
\textbf{Notă:} \texttt{crypto/rand} din Go este un CSPRNG (Cryptographically Secure PRNG).
\end{frame}
...
\begin{frame}{Ce am Realizat}
    ...
    \begin{enumerate}
        ...
        \item ✓ Generare cheie securizată (Go crypto/rand)
...
\begin{frame}{Resurse și Referințe}
    ...
    \begin{itemize}
        \item Go Documentation: crypto/rand package
